...
COMMENT ON COLUMN SEEK_ANALYSIS.ANALYSIS_HASH IS '분석_해시';
...
CREATE TABLE SEEK_DATABASE_OPERATION
(
  OPERATION_HIST_ID  CHAR(36) NOT NULL,
  TARGET_INFORMATION TEXT     NOT NULL,
  CONTENT            TEXT    ,
  PRIMARY KEY (OPERATION_HIST_ID)
);

COMMENT ON TABLE SEEK_DATABASE_OPERATION IS '데이터베이스_작업';

COMMENT ON COLUMN SEEK_DATABASE_OPERATION.OPERATION_HIST_ID IS '작업_이력_ID';

COMMENT ON COLUMN SEEK_DATABASE_OPERATION.TARGET_INFORMATION IS '대상_정보';

COMMENT ON COLUMN SEEK_DATABASE_OPERATION.CONTENT IS '내용';
...
CREATE TABLE SEEK_OPERATION_HIST
(
  OPERATION_HIST_ID  CHAR(36)    NOT NULL,
...
COMMENT ON TABLE SEEK_OPERATION_HIST IS '작업_이력';

COMMENT ON COLUMN SEEK_OPERATION_HIST.OPERATION_HIST_ID IS '작업_이력_ID';

COMMENT ON COLUMN SEEK_OPERATION_HIST.OPERATION_TYPE_CCD IS '작업_타입_공통코드';

COMMENT ON COLUMN SEEK_OPERATION_HIST.ANALYSIS_ID IS '분석_ID';

COMMENT ON COLUMN SEEK_OPERATION_HIST.CREATE_DT IS '생성_일시';

CREATE TABLE SEEK_PROXY_OPERATION
(
  OPERATION_HIST_ID CHAR(36) NOT NULL,
...
COMMENT ON TABLE SEEK_PROXY_OPERATION IS '프록시_작업';

COMMENT ON COLUMN SEEK_PROXY_OPERATION.OPERATION_HIST_ID IS '작업_이력_ID';

COMMENT ON COLUMN SEEK_PROXY_OPERATION.OPERATION_ID IS '작업_ID';

COMMENT ON COLUMN SEEK_PROXY_OPERATION.URL IS 'URL';

COMMENT ON COLUMN SEEK_PROXY_OPERATION.HEADER IS '헤더';

COMMENT ON COLUMN SEEK_PROXY_OPERATION.QUERY_STRING IS '쿼리_문자열';

COMMENT ON COLUMN SEEK_PROXY_OPERATION.BODY IS '바디';

COMMENT ON COLUMN SEEK_PROXY_OPERATION.FILE_ID IS '파일_ID';
...
ALTER TABLE SEEK_PROXY_OPERATION
  ADD CONSTRAINT FK_SEEK_OPERATION_HIST_TO_SEEK_PROXY_OPERATION
    FOREIGN KEY (OPERATION_HIST_ID)
    REFERENCES SEEK_OPERATION_HIST (OPERATION_HIST_ID);

ALTER TABLE SEEK_DATABASE_OPERATION
  ADD CONSTRAINT FK_SEEK_OPERATION_HIST_TO_SEEK_DATABASE_OPERATION
    FOREIGN KEY (OPERATION_HIST_ID)
    REFERENCES SEEK_OPERATION_HIST (OPERATION_HIST_ID);

ALTER TABLE SEEK_OPERATION_HIST
  ADD CONSTRAINT FK_SEEK_ANALYSIS_TO_SEEK_OPERATION_HIST
    FOREIGN KEY (ANALYSIS_ID)
    REFERENCES SEEK_ANALYSIS (ANALYSIS_ID);

CREATE INDEX IX_SEEK_ANALYSIS_HASH
  ON SEEK_ANALYSIS (ANALYSIS_HASH ASC);
