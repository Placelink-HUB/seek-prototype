...
CREATE TABLE SEEK_FILE_DETECTION
(
  ANALYSIS_ID    CHAR(36) NOT NULL,
  FILE_ID        CHAR(36) NOT NULL,
...
COMMENT ON TABLE SEEK_FILE_DETECTION IS '파일_탐지';

COMMENT ON COLUMN SEEK_FILE_DETECTION.ANALYSIS_ID IS '작업_ID';

COMMENT ON COLUMN SEEK_FILE_DETECTION.FILE_ID IS '파일_ID';

COMMENT ON COLUMN SEEK_FILE_DETECTION.SIGNED_FILE_ID IS '서명된_파일_ID';

COMMENT ON COLUMN SEEK_FILE_DETECTION.REQUESTER_UID IS '요청자_식별자';
...
ALTER TABLE SEEK_FILE_DETECTION
  ADD CONSTRAINT FK_SEEK_ANALYSIS_TO_SEEK_FILE_DETECTION
    FOREIGN KEY (ANALYSIS_ID)
    REFERENCES SEEK_ANALYSIS (ANALYSIS_ID);
